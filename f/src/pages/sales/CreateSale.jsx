...
const CreateSale = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5555/products/`,{params:user}).then((response) => {
  ...
  const handleCreateSale =  async() => {
    ...
    const saleData = {
      products: productSales.map((productSale) => ({
        product: productSale.product,
        quantity: parseInt(productSale.quantity, 10), // Parse quantity as an integer
    ...
    await axios
      .post(`http://localhost:5555/sales/`, saleData);
  ...
  return (
    <div className='p-4'>
      ...
      <div className='flex flex-col border-2 border-sky-400 rounded-xl w-[600px] p-4 mx-auto'>
        ...
        <button className='p-2 bg-sky-300 m-8' onClick={handleCreateSale}>
          Save
        </button>
      </div>
    </div>
